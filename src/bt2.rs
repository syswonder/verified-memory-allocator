    ...
    fn find_contiguous(&self, capacity: usize, size: usize, align_log2: usize) -> (res: Option<usize>) {
        ...
        while offset < capacity
            invariant
                ...
                offset <= capacity,
                offset - base < size,
                forall|i: int| (0 <= i < base) ==> has_obstruction(self@, i, size as int, (1usize << align_log2) as int),
                forall|i: int| (0 <= i < capacity) && (i % (1usize << align_log2) as int != 0) ==> has_obstruction(self@, i, size as int, (1usize << align_log2) as int),
                capacity == Self::spec_cap(),
                align_log2 < 64,
                0 < size <= capacity,
                0 <= base <= offset,
                base % (1usize << align_log2) == 0,
                forall|i: int| base <= i < offset ==> self@.index(i),
            decreases
                capacity - offset,
        {
            if let Some(next) = self.next(offset) {
                assert(next < Self::spec_cap());
                assert(offset - base < size);
                assert(next >= offset);
                assert(self@[next as int] == true);
                if next != offset {
                    assert(next > offset);
                    ...
                    assert(self@[offset as int] == false);
                    assert(forall|i: usize| (offset - size < i <= offset) ==> has_obstruction(self@, i as int, size as int, (1usize << align_log2) as int));
                    ...
                    assert(forall|i: int| (offset <= i < next) ==> (self@[i] == false));
                    // lemma_bit_false_implies_has_obstruction(ba@,);
                    assert forall|i: usize| (offset <= i < next) implies has_obstruction(self@, i as int, size as int, (1usize << align_log2) as int) by{
                    ...
                    assert(offset - base < size);
                    assert forall|i: usize| (next <= i < base) implies has_obstruction(self@, i as int, size as int, (1usize << align_log2) as int) by{
                ...
                assert(self@[offset as int] == false);
                assert(forall|i: usize| (offset - size < i <= offset) ==> has_obstruction(self@, i as int, size as int, (1usize << align_log2) as int));
                assert forall|i: usize| (offset <= i < capacity) implies has_obstruction(self@, i as int, size as int, (1usize << align_log2) as int) by{
                    assert(self@[i as int] == false);
                }
                return None;
            }
